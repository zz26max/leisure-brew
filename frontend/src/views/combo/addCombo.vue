  ...
  private async init() {
    querySetmealById(this.$route.query.id).then(res => {
      if (res && res.data && res.data.code === 1) {
        ...
        this.checkList = res.data.data.comboDrinks
        this.dishTable = res.data.data.comboDrinks.reverse()
  ...
  public submitForm(formName: any, st: any) {
    (this.$refs[formName] as any).validate((valid: any) => {
      if (valid) {
        ...
        prams.comboDrinks = this.dishTable.map((obj: any) => ({
          copies: obj.copies,
          dishId: obj.dishId,
          name: obj.name,
          price: obj.price
